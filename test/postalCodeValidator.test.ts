import { countryHasPostalCodeSystem, isValidPostalCode, isValidCountryPostalCode } from "../src/postalCodeValidator";
import {
...
describe('countryHasPostalCodeRules', () => {
    test.each(Object.keys(CountryWithNoPostalCode))(
        ...
        (countryKey) => {
            const country = CountryWithNoPostalCode[countryKey as keyof typeof CountryWithNoPostalCode];
            expect(countryHasPostalCodeSystem(castCountryWithNoPostalCode(country))).toBe(false);
        }
    );
    ...
    test.each(Object.keys(CountryWithPostalCode))(
        ...
        (countryKey) => {
            const country = CountryWithPostalCode[countryKey as keyof typeof CountryWithPostalCode];
            expect(countryHasPostalCodeSystem(castCountryWithPostalCode(country))).toBe(true);
        }
    );
});
